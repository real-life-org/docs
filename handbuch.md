# Real Life Network — Handbuch

Ein praktischer Leitfaden für Menschen, die beitragen wollen.

---

## Du bist Teil des Netzwerks

Es gibt keinen Beitritt, keine Anmeldung, keinen Mitgliedsausweis. Du bist Teil des Netzwerks, wenn du dich dazugehörig fühlst und aktiv wirst — egal wie klein der Beitrag ist.

Dieses Handbuch zeigt dir, was du konkret tun kannst. Nicht alles, nicht sofort, nicht alleine. Sondern das, was sich für dich richtig anfühlt. Das Netzwerk entsteht aus der Summe vieler kleiner Beiträge von vielen verschiedenen Menschen.

---

## So entsteht das Netzwerk

Niemand steuert das Netzwerk von oben. Es gibt keinen Plan, der abgearbeitet wird. Stattdessen passiert jeden Tag folgendes:

- Jemand lädt eine Freundin zu einem Circle ein
- Jemand lernt auf einem Festival eine Initiative kennen und trägt sie auf der Karte ein
- Jemand bringt einen Kuchen zum Treffen mit
- Jemand macht ein Foto und teilt es
- Jemand stellt zwei Menschen vor, die sich kennen sollten

Keine dieser Handlungen ist spektakulär. Aber zusammen ergeben sie ein lebendiges Netzwerk. Dein Beitrag muss nicht groß sein — er muss nur echt sein.

---

## Was du tun kannst

### Spontan

- **Jemanden einladen** — zu einem Circle, einem Festival, ins Netzwerk
- **Jemanden willkommen heißen**, der neu ist — der erste Eindruck zählt
- **Einen Kontakt weitergeben** — "Du solltest X kennenlernen, ihr arbeitet am gleichen Thema"
- **Ein Vertrauensverhältnis bestätigen** — im Web of Trust, bei einer echten Begegnung
- **Zuhören** — manchmal ist das der wichtigste Beitrag

### Regelmäßig

- **Einen Circle planen** — den Raum halten, für Atmosphäre sorgen, alle einbeziehen
- **Termine pflegen** — Circle-Treffen, lokale Events im Kalender eintragen
- **Gemeinsame Ressourcen verwalten** — wer hat gerade das Zelt? Wo ist die Kochausrüstung?
- **Neue Einträge auf der Karte anlegen** — Initiativen, Projekte, Orte in deiner Umgebung sichtbar machen
- **Geschichten erzählen** — ein Foto vom Circle, ein kurzer Text über eine Begegnung, ein Video von einem Festival

### Bei Gelegenheit

- **Auf ein Festival fahren** und das Vernetzungszelt betreiben
- **Einen neuen Circle anstoßen** — in deiner Stadt oder einer anderen
- **Eine bestehende Initiative ansprechen** — SoLaWi, Transition Town, Repair Café — und eine Brücke ins Netzwerk bauen
- **Equipment transportieren** — Zelt, Material, Küche dorthin bringen, wo es gebraucht wird
- **Jemanden einladen**, der das Netzwerk noch nicht kennt

### Im Hintergrund

- **Die App weiterentwickeln** — Code, Design, Testing
- **Server und Infrastruktur pflegen** — damit die Werkzeuge laufen
- **Förderanträge schreiben** — Mittel einwerben für Equipment, Reisen, Festivals
- **Erfahrungen dokumentieren** — damit andere Circles von euren Learnings profitieren
- **Materialien gestalten** — Flyer, Poster, Vorlagen für neue Circles

---

## Einen Circle starten

### Was du brauchst

- 3–5 Menschen, denen du vertraust
- Einen Ort (Wohnzimmer, Park, Gemeinschaftsraum — alles geht)
- Einen festen Rhythmus (alle 2 Wochen funktioniert gut)

### Das erste Treffen

1. **Ankommen** — gebt euch 10 Minuten zum Ankommen, Tee machen, Platz nehmen
2. **Check-in** — jeder sagt kurz, wie es ihm geht und mit welcher Absicht er da ist. Keine Diskussion, nur Zuhören.
3. **Eine Frage in die Mitte legen** — z.B. "Was bedeutet Gemeinschaft für mich?" oder "Was brauche ich gerade?" Reihum sprechen, keine Unterbrechung.
4. **Austausch** — offenes Gespräch über das, was aufkam
5. **Check-out** — jeder sagt einen Satz zum Abschluss. "Was nehme ich mit?"

Dauer: 1,5 bis 2 Stunden reichen.

### Was danach kommt

- Einigt euch auf den nächsten Termin bevor ihr auseinandergeht
- Tragt den Circle im Kalender der App ein
- Ladet beim nächsten Mal ein oder zwei neue Leute ein
- Experimentiert mit dem Format — es gehört euch

### Was einen Circle am Leben hält

- **Regelmäßigkeit** — der Rhythmus ist wichtiger als die Perfektion
- **Offenheit** — neue Menschen sind willkommen
- **Keine Agenda** — der Circle gehört den Menschen darin, nicht einem Programm
- **Gemeinsames Essen** — zusammen kochen oder jeder bringt etwas mit. Essen verbindet.
- **Geduld** — es braucht 3–4 Treffen, bis eine Gruppe ihren Rhythmus findet

---

## Auf einem Festival

### Vorbereitung

- **Equipment checken** — Zelt/Pavillon, Sitzgelegenheiten, Moderationsmaterial, Karte (analog oder digital), Banner
- **Team zusammenstellen** — 3–5 Menschen, die sich abwechseln können
- **Format vorbereiten** — nicht zu viel planen, aber ein paar Gesprächsformate im Kopf haben
- **Anreise organisieren** — wer fährt, wer nimmt Material mit

### Vor Ort

- **Einen einladenden Raum schaffen** — Sitzkreis, Tee, Snacks, offene Atmosphäre
- **Nicht am Stand stehen und warten** — aktiv auf Menschen zugehen, ins Gespräch kommen
- **Die Karte nutzen** — "Wo kommst du her? Was machst du?" Jede Begegnung auf der Karte festhalten
- **Formate anbieten** — Check-in Runde, Gesprächskreis, gemeinsames Kochen
- **Kontakte machen** — Web of Trust Attestations nach echten Gesprächen
- **Keine Flyer in die Hand drücken** — die Erfahrung ist der beste Flyer

### Danach

- **Kontakte nachfassen** — persönliche Nachricht an die Menschen, die Interesse gezeigt haben
- **Regionale Verbindungen anstoßen** — "5 Leute aus Leipzig haben sich bei uns getroffen — wollt ihr einen Circle starten?"
- **Erfahrungen teilen** — was hat funktioniert, was nicht? Fotos, kurzer Bericht fürs Netzwerk
- **Equipment zurückbringen** — oder an den nächsten Festival-Einsatz weitergeben

---

## Ressourcen und Commons

### Der gemeinsame Pool

Das Netzwerk baut einen wachsenden Pool an geteilten Ressourcen auf. Alles was eingebracht wird — ob gekauft, gespendet oder mitgebracht — steht der Gemeinschaft zur Verfügung.

**So funktioniert es:**
- Jemand bringt einen Pavillon mit → er wird im Marktplatz eingetragen, die Person verwaltet ihn
- Das Netzwerk kauft eine Kochausrüstung von Fördermitteln → sie wird eingetragen, jemand übernimmt die Verwaltung
- Jemand bietet seinen Garten für Circle-Treffen an → erscheint auf der Karte

**Verwaltung heißt:** Du weißt wo es ist, du hältst es in Schuss, du koordinierst wer es wann nutzt. Es gehört nicht dir — du kümmerst dich darum.

### Transparenz

Im Marktplatz der App ist sichtbar:
- Was das Netzwerk gemeinsam hat
- Wer was verwaltet
- Wie man es nutzen kann

### Der Commons-Topf

Es gibt einen gemeinsamen Topf für Geld. Wer etwas geben kann, gibt. Wer nicht, gibt nicht. Kein Betrag, kein Druck.

Was im Topf ist, wird transparent gemacht. Ausgaben werden gemeinsam besprochen. Geld wird zu Gegenständen, Gegenstände werden zu Commons.

---

## Entscheidungen

### Im Alltag: Free Flow

Die meisten Dinge regeln sich, wenn Menschen mit guter Absicht handeln. Du willst einen Circle starten? Tu es. Du willst ein Item auf der Karte anlegen? Tu es. Du willst auf ein Festival fahren? Tu es.

### Wenn es andere betrifft: Konsent

Wenn eine Entscheidung andere im Netzwerk betrifft, suchen wir einen Konsent: Wir handeln, solange niemand einen schwerwiegenden Einwand hat. Ein schwerwiegender Einwand ist nicht "ich hätte es anders gemacht" sondern "das richtet Schaden an."

### Bei grundlegenden Fragen: RFC

Für Entscheidungen, die das ganze Netzwerk oder die Organisation betreffen, gibt es einen offenen RFC-Prozess. Jemand schreibt einen Vorschlag, alle können kommentieren, nach einer Frist wird entschieden. Alles dokumentiert und nachvollziehbar.

---

## Die Werkzeuge

Das Netzwerk hat eigene digitale Werkzeuge. Sie sind optional — das Netzwerk lebt durch Begegnungen, nicht durch Software. Aber sie machen vieles einfacher.

**Web of Trust** — Nach einer echten Begegnung bestätigt ihr euch gegenseitig. So entsteht ein Netz aus verifizierten Vertrauensbeziehungen.

**Karte** — Zeigt was es in deiner Umgebung gibt: Circles, Initiativen, geteilte Ressourcen, Menschen.

**Kalender** — Alle Treffen und Events an einem Ort. Dein Circle trägt seine Termine ein, du siehst was sonst noch passiert.

**Marktplatz** — Wer hat was? Ein Zelt, eine Fähigkeit, einen Raum? Hier findest du es.

**App herunterladen:** [F-Droid](https://fdroid.utopia-lab.org/fdroid/repo)

---

*Dieses Handbuch wächst mit dem Netzwerk. Wenn du etwas gelernt hast, das anderen helfen könnte — trag es ein.*
